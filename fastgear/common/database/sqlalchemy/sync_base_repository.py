...
class BaseRepository(AbstractRepository[EntityType]):
    ...
    def __init__(self, entity: type[EntityType], db: SyncSessionType) -> None:
        super().__init__(entity, db)
        self.select_constructor = SelectConstructor(entity)
        self.repo_utils = BaseRepositoryUtils()

    def create(self, new_record: EntityType | BaseModel) -> EntityType:
        ...
        """
        return (self.create_all([new_record]))[0]

    def create_all(self, new_records: list[EntityType | BaseModel]) -> list[EntityType]:
    ...
    def save(
        ...
        if new_record:
            self.refresh_record(new_record)
        return new_record
    ...
    def commit_or_flush(self) -> None:
        ...
        Returns:
            None
        """
        self.db.flush() if self.db.in_nested_transaction() else self.db.commit()
    ...
    def refresh_record(
        ...
        (
            ...
            else (self.db.refresh(_entity) for _entity in new_record)
        )
        return new_record
    ...
    def find_one(self, search_filter: str | FindOneOptions) -> EntityType | None:
        ...
        result = self.db.execute(select_statement).first()
    ...
    def find_one_or_fail(self, search_filter: str | FindOneOptions) -> EntityType:
        ...
        select_statement = self.select_constructor.build_select_statement(search_filter)
        try:
            result = self.db.execute(select_statement).one()[0]
    ...
    def find(self, stmt_or_filter: FindManyOptions | Select = None) -> Sequence[EntityType]:
    ...
    @find.register
    def _(self, options: FindManyOptions | dict | None) -> Sequence[EntityType]:
        ...
        return self.find(select_statement)  # Call the method registered for Select

    @find.register
    def _(self, select_stmt: Select) -> Sequence[EntityType]:
        """Implementation when stmt_or_filter is an instance of Select."""
        return self.db.execute(select_stmt).scalars().all()

    @singledispatchmethod
    def count(self, stmt_or_filter: FindManyOptions | Select = None) -> int:
    ...
    @count.register
    def _(self, options: FindManyOptions | dict | None) -> int:
        """Implementation when stmt_or_filter is an instance of FindManyOptions."""
        select_statement = self.select_constructor.build_select_statement(options)
        return self.count(select_statement)

    @count.register
    def _(self, select_stmt: Select) -> int:
        """Implementation when stmt_or_filter is an instance of Select."""
    ...
    def find_and_count(
        ...
        """
        select_statement = self.select_constructor.build_select_statement(search_filter)
        count = self.count(select_statement)
        result = self.find(select_statement)
    ...
    def update(
        ...
        """
        record = self.find_one_or_fail(search_filter)
    ...
    def delete(self, delete_statement: str | FindOneOptions | ReturningDelete) -> DeleteResult:
        ...
        if isinstance(delete_statement, Delete):
            raw = self.db.execute(delete_statement).all()
        else:
            record = self.find_one_or_fail(delete_statement)
