...
from fastgear.utils import EnvBaseSettings, TomlBaseSettings
...
@pytest.mark.describe("🧪  EnvBaseSettings")
class TestEnvBaseSettings:
    @pytest.mark.it("✅  Should customize settings sources correctly")
    def test_settings_customise_sources(
        self, settings_sources: dict[str, PydanticBaseSettingsSource]
    ) -> None:
        sources = EnvBaseSettings.settings_customise_sources(
            EnvBaseSettings,
            settings_sources["init_settings"],
            settings_sources["env_settings"],
            settings_sources["dotenv_settings"],
        ...
        assert len(sources) == 2
        assert sources[0] == settings_sources["env_settings"]
        assert isinstance(sources[1], DotEnvSettingsSource)
        assert sources[1].settings_cls == EnvBaseSettings

    @pytest.mark.it("✅  Should prioritize environment variables over dotenv config")
    def test_settings_source_priority(
        self, settings_sources: dict[str, PydanticBaseSettingsSource]
    ) -> None:
        sources = EnvBaseSettings.settings_customise_sources(
...
@pytest.mark.describe("🧪  EnvBaseSettings.get_env_files")
class TestGetEnvFiles:
    @pytest.mark.it("✅  Should return empty list when config directory does not exist")
    def test_returns_empty_list_when_dir_not_exists(
        self, temp_config_dir: Path, env_enum: type[StrEnum]
    ) -> None:
        result = EnvBaseSettings.get_env_files(temp_config_dir, env_enum)

        assert result == []

    @pytest.mark.it("✅  Should return only base dotenv files when they exist")
    def test_returns_base_dotenv_files(
        self, config_dir_with_base_dotenv_files: Path, env_enum: type[StrEnum]
    ) -> None:
        result = EnvBaseSettings.get_env_files(config_dir_with_base_dotenv_files, env_enum)

        assert len(result) == 2
        assert str(config_dir_with_base_dotenv_files / ".env") in result
        assert str(config_dir_with_base_dotenv_files / ".env.local") in result

    @pytest.mark.it("✅  Should return environment-specific dotenv files")
    def test_returns_env_specific_files(
        self, config_dir_with_env_dotenv_files: Path, env_enum: type[StrEnum]
    ) -> None:
        result = EnvBaseSettings.get_env_files(config_dir_with_env_dotenv_files, env_enum)

        assert str(config_dir_with_env_dotenv_files / ".env.dev") in result
        assert str(config_dir_with_env_dotenv_files / ".env.dev.local") in result
        assert str(config_dir_with_env_dotenv_files / ".env.prod") in result

    @pytest.mark.it("✅  Should return all dotenv files in correct order")
    def test_returns_all_files_in_order(
        self, config_dir_with_all_dotenv_files: Path, env_enum: type[StrEnum]
    ) -> None:
        result = EnvBaseSettings.get_env_files(config_dir_with_all_dotenv_files, env_enum)

        assert len(result) == 6
        assert result[0] == str(config_dir_with_all_dotenv_files / ".env")
        assert result[1] == str(config_dir_with_all_dotenv_files / ".env.local")
        assert result[2] == str(config_dir_with_all_dotenv_files / ".env.dev")
        assert result[3] == str(config_dir_with_all_dotenv_files / ".env.dev.local")
        assert result[4] == str(config_dir_with_all_dotenv_files / ".env.prod")
        assert result[5] == str(config_dir_with_all_dotenv_files / ".env.prod.local")
    ...
    def test_skips_duplicate_paths_from_enum(
        self,
        config_dir_with_duplicate_dotenv_candidates: Path,
        duplicate_dotenv_enum: type[StrEnum],
    ) -> None:
        result = EnvBaseSettings.get_env_files(
            config_dir_with_duplicate_dotenv_candidates, duplicate_dotenv_enum
        )

        assert len(result) == 2
        assert str(config_dir_with_duplicate_dotenv_candidates / ".env") in result
        assert str(config_dir_with_duplicate_dotenv_candidates / ".env.local") in result
        assert result.count(str(config_dir_with_duplicate_dotenv_candidates / ".env.local")) == 1
